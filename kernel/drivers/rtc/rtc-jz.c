		...
		unsigned long arg)
{
	...
	switch (cmd) {
	case JZ_RTC_HIBERNATE_STATUS: {
	    unsigned int hwrsr = jzrtc_readl(rtc, RTC_HWRSR);
	    unsigned int status = 0;

	    if (hwrsr & (HWRSR_APD)) {
	        status = JZ_RTC_HIBERNATE_APD;

	    } else if (hwrsr & (HWRSR_HR)) {
	        if (hwrsr & (HWRSR_PIN))
	            status = JZ_RTC_HIBERNATE_PIN;
	        else if (hwrsr & (HWRSR_ALM))
	            status = JZ_RTC_HIBERNATE_ALM;

	    } else if (hwrsr & (HWRSR_PPR)) {
	        status = JZ_RTC_HIBERNATE_PPR;
	    } else
	        status = JZ_RTC_HIBERNATE_UNKNOWN;

        err = put_user(status, (unsigned long __user *)arg);
        if (!err) {
            /* clear all rtc flags */
