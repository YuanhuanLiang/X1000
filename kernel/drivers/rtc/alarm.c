 ...
 */
__attribute__((unused)) static void alarm_init(struct alarm *alarm,
 ...
 */
static int alarm_try_to_cancel(struct alarm *alarm)
{
	struct alarm_queue *base = &alarms[alarm->type];
	unsigned long flags;
	bool first = false;
 ...
 *  1 when the alarm was active
 */
__attribute__((unused)) static int alarm_cancel(struct alarm *alarm)
{
	for (;;) {
