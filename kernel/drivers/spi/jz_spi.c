...
{
#if defined(CONFIG_JZ_SPI_PIO_CE)
	struct spi_cs_ctrl *cs_ctrl = spi->chipselect + cs;
	u32 pin_value = cs_ctrl->cs_pin;
	if (cs_ctrl->hold_cs && gpio_is_valid(pin_value)) {
//printk(" gpio: %d value: %d\n", pin_value, (!pol ? 0 : 1)); // -- add liangyh
...
static int jz_spi_set_clk(struct spi_device *spi, u32 hz)
{
	...
	if (hw->clk_setting == hz)
		return 0;

#ifdef CONFIG_JZ_SPI_CLK_SOURCE_EXTERNAL
	ssi_clk_rate = clk_get_rate(hw->clk_gate);
#else
	ssi_clk_rate = clk_get_rate(hw->clk) / 2;
#endif
	if (hz > ssi_clk_rate / 2) {
	    cgv = 0;
	    dev_warn(hw->dev, "The ssi device rate more than %ld\n", ssi_clk_rate / 2);

	} else if (hz < ssi_clk_rate / (2 *(0xff + 1))) {
	    cgv = 0xff;
	    dev_warn(hw->dev, "The ssi device rate less than %ld\n", ssi_clk_rate / (2 *(0xff + 1)));

	} else {
	    cgv = ssi_clk_rate / (2 * hz) - 1;
	}
...
#ifdef JZ_SPI_CLK_SOURCE_EXTERNAL
	rate = clk_get_rate(hw->clk_gate) / (2 * (cgv + 1));
#else
	rate = clk_get_rate(hw->clk) / 2 / (2 * (cgv + 1));
#endif
...
static int jz_spi_setupxfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	if (hw->use_dma) {// use this --liangyh
...
static int __init jz_spi_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < hw->pdata->num_chipselect; i++, num_cs_got = i) {
		if (gpio_is_valid(hw->pdata->chipselect[i].cs_pin)) {
			err = gpio_request(hw->pdata->chipselect[i].cs_pin, "JZ_SPI_CS");
			if(err && (!hw->pdata->allow_cs_same)) {
				dev_err(&pdev->dev, "Request cs_gpio: %d is occupied\n",
								hw->pdata->chipselect[i].cs_pin);
				goto err_cs_gpio;
			}
		}
	}
...
err_cs_gpio:
	for (i = 0; i < num_cs_got; i++)
		gpio_free(hw->pdata->chipselect[i].cs_pin);
#endif
err_no_pdata:
...
static int __exit jz_spi_remove(struct platform_device *dev)
{
	...
	if(hw->clk && clk_is_enabled(hw->clk))
		clk_disable(hw->clk);
