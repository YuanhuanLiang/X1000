...
struct uart_jz47xx_port {
	...
	struct jz_uart_platform_data *uart0_pdata;

#ifdef CONFIG_WAKEUP_SOURCE_UART0
	int uart0_gpio_irq;
#endif
    ...
    struct jz_uart_platform_data *uart1_pdata;

#ifdef CONFIG_WAKEUP_SOURCE_UART1
    int uart1_gpio_irq;
#endif
    ...
    struct jz_uart_platform_data *uart2_pdata;

#ifdef CONFIG_WAKEUP_SOURCE_UART2
    int uart2_gpio_irq;
#endif
...
#if (defined CONFIG_WAKEUP_SOURCE_UART0) || (defined CONFIG_WAKEUP_SOURCE_UART1) || (defined CONFIG_WAKEUP_SOURCE_UART2)
static inline irqreturn_t jz_uart_gpio_irq(int irq, void *dev_id) {
    struct uart_jz47xx_port *up = dev_id;

    dev_info(up->port.dev, "uart%d gpio irq\n", up->port.line);
...
#ifdef CONFIG_PM
#if ((defined CONFIG_WAKEUP_SOURCE_UART0) || \
	 (defined CONFIG_WAKEUP_SOURCE_UART1) || \
	 (defined CONFIG_WAKEUP_SOURCE_UART2))

static void serial_pin_set_func(int gpio, enum gpio_function func,
        int enable_pull) {
    int port = gpio / 32;
    int pin = BIT(gpio & 0x1f);

    if (enable_pull)
        jzgpio_set_func_pull(port, func, pin);
    else
        jzgpio_set_func_nopull(port, func, pin);
}
#endif
...
#ifdef CONFIG_WAKEUP_SOURCE_UART0
    enum gpio_function func0;
    if (up->port.line == 0 && device_may_wakeup(dev)) {
        if (up->uart0_pdata->wakeup_pin->trigger_edge == IRQF_TRIGGER_FALLING)
            func0 = GPIO_INT_FE;
        else
            func0 = GPIO_INT_RE;
        serial_pin_set_func(up->uart0_pdata->wakeup_pin->num, func0, 1);
        enable_irq(up->uart0_gpio_irq);
        enable_irq_wake(up->uart0_gpio_irq);
    }
#endif

#ifdef CONFIG_WAKEUP_SOURCE_UART1
    enum gpio_function func1;
    if (up->port.line == 1 && device_may_wakeup(dev)) {
        if (up->uart1_pdata->wakeup_pin->trigger_edge == IRQF_TRIGGER_FALLING)
            func1 = GPIO_INT_FE;
        else
            func1 = GPIO_INT_RE;
        serial_pin_set_func(up->uart1_pdata->wakeup_pin->num, func1, 1);
        enable_irq(up->uart1_gpio_irq);
        enable_irq_wake(up->uart1_gpio_irq);
    }
#endif

#ifdef CONFIG_WAKEUP_SOURCE_UART2
    enum gpio_function func2;
    if (up->port.line == 2 && device_may_wakeup(dev)) {
        if (up->uart2_pdata->wakeup_pin->trigger_edge == IRQF_TRIGGER_FALLING)
            func2 = GPIO_INT_FE;
        else
            func2 = GPIO_INT_RE;
        serial_pin_set_func(up->uart2_pdata->wakeup_pin->num, func2, 1);
        enable_irq(up->uart2_gpio_irq);
        enable_irq_wake(up->uart2_gpio_irq);
    }
#endif
...
#ifdef CONFIG_WAKEUP_SOURCE_UART0
    if (up->port.line == 0 && device_may_wakeup(dev)) {
        disable_irq_nosync(up->uart0_gpio_irq);
        serial_pin_set_func(up->uart0_pdata->wakeup_pin->num,
                up->uart0_pdata->wakeup_pin->def_func, 1);
        disable_irq_wake(up->uart0_gpio_irq);
    }
#endif

#ifdef CONFIG_WAKEUP_SOURCE_UART1
    if (up->port.line == 1 && device_may_wakeup(dev)) {
        disable_irq_nosync(up->uart1_gpio_irq);
        serial_pin_set_func(up->uart1_pdata->wakeup_pin->num,
                up->uart1_pdata->wakeup_pin->def_func, 1);
        disable_irq_wake(up->uart1_gpio_irq);
    }
#endif

#ifdef CONFIG_WAKEUP_SOURCE_UART2
    if (up->port.line == 2 && device_may_wakeup(dev)) {
        disable_irq_nosync(up->uart2_gpio_irq);
        serial_pin_set_func(up->uart2_pdata->wakeup_pin->num,
                up->uart2_pdata->wakeup_pin->def_func, 1);
        disable_irq_wake(up->uart2_gpio_irq);
    }
#endif
...
#ifdef CONFIG_WAKEUP_SOURCE_UART0
    if (up->port.line == 0) {
        up->uart0_pdata = dev->dev.platform_data;

        if (up->uart0_pdata && up->uart0_pdata->wakeup_pin != NULL) {
            device_init_wakeup(&dev->dev, 1);

            ret = gpio_request(up->uart0_pdata->wakeup_pin->num, "uart0-wakeup");
            if (ret) {
                dev_err(&dev->dev, "Failed to request wakeup-gpio for uart0\n");
                return ret;
            }

            up->uart0_gpio_irq = gpio_to_irq(up->uart0_pdata->wakeup_pin->num);

            ret = request_any_context_irq(up->uart0_gpio_irq, jz_uart_gpio_irq,
                    up->uart0_pdata->wakeup_pin->trigger_edge | IRQF_DISABLED,
                    dev_name(&dev->dev), up);
            if (ret) {
                dev_err(&dev->dev, "Failed to request irq for uart0\n");
                return ret;
            }
            disable_irq_nosync(up->uart0_gpio_irq);
            dev_info(up->port.dev, "uart%d gpio wakeup init done.\n", up->port.line);
        } else
            dev_info(up->port.dev, "uart%d gpio wakeup init fail.\n", up->port.line);
    }
#endif

#ifdef CONFIG_WAKEUP_SOURCE_UART1
    if (up->port.line == 1) {
        up->uart1_pdata = dev->dev.platform_data;

        if (up->uart1_pdata != NULL && up->uart1_pdata->wakeup_pin != NULL) {
            device_init_wakeup(&dev->dev, 1);

            ret = gpio_request(up->uart1_pdata->wakeup_pin->num, "uart1-wakeup");
            if (ret) {
                dev_err(&dev->dev, "Failed to request wakeup-gpio for uart1\n");
                return ret;
            }

            up->uart1_gpio_irq = gpio_to_irq(up->uart1_pdata->wakeup_pin->num);

            ret = request_any_context_irq(up->uart1_gpio_irq, jz_uart_gpio_irq,
                    up->uart1_pdata->wakeup_pin->trigger_edge | IRQF_DISABLED,
                    dev_name(&dev->dev), up);
            if (ret) {
                dev_err(&dev->dev, "Failed to request irq for uart1\n");
                return ret;
            }
            disable_irq_nosync(up->uart1_gpio_irq);
            dev_info(up->port.dev, "uart%d gpio wakeup init done.\n", up->port.line);
...
#ifdef CONFIG_WAKEUP_SOURCE_UART2
    if (up->port.line == 2) {
        up->uart2_pdata = dev->dev.platform_data;

        if (up->uart2_pdata && up->uart2_pdata->wakeup_pin != NULL) {
            device_init_wakeup(&dev->dev, 1);

            ret = gpio_request(up->uart2_pdata->wakeup_pin->num, "uart2-wakeup");
            if (ret) {
                dev_err(&dev->dev, "Failed to request wakeup-gpio for uart2\n");
                return ret;
            }

            up->uart2_gpio_irq = gpio_to_irq(up->uart2_pdata->wakeup_pin->num);

            ret = request_any_context_irq(up->uart2_gpio_irq, jz_uart_gpio_irq,
                    up->uart2_pdata->wakeup_pin->trigger_edge | IRQF_DISABLED,
                    dev_name(&dev->dev), up);
            if (ret) {
                dev_err(&dev->dev, "Failed to request irq for uart2\n");
                return ret;
            }
            disable_irq_nosync(up->uart2_gpio_irq);
            dev_info(up->port.dev, "uart%d gpio wakeup init done.\n", up->port.line);
...
#ifdef CONFIG_WAKEUP_SOURCE_UART0
    free_irq(up->uart0_gpio_irq, up);
    gpio_free(up->uart0_pdata->wakeup_pin->num);
#endif

#ifdef CONFIG_WAKEUP_SOURCE_UART1
    free_irq(up->uart1_gpio_irq, up);
    gpio_free(up->uart1_pdata->wakeup_pin->num);
#endif

#ifdef CONFIG_WAKEUP_SOURCE_UART2
    free_irq(up->uart2_gpio_irq, up);
    gpio_free(up->uart2_pdata->wakeup_pin->num);
#endif
