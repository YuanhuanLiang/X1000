...
static int jz_pwm_request(struct pwm_chip *chip, struct pwm_device *pwm)
{
...
#ifdef CONFIG_BATTERY_PWM
    if (CONFIG_BATTERY_PWM_INDEX == id)
        tcu_pwm->clock = PCLK_EN;
#endif
	...
	if(tcu_as_timer_init(tcu_pwm) != 0)
		return -EINVAL;

	printk("%s, request pwm channel %d successfully\n", __func__, id);
	return 0;
}
			     ...
			     int duty_ns, int period_ns)
{
	...
	tcu_pwm->id = id;
	tcu_pwm->init_level = !pwm->active_level;
	tcu_pwm->pwm_flag = pwm->flags;
...
#ifdef CONFIG_BATTERY_PWM
	jz_pwm->pclk = clk_get(&pdev->dev, "pclk");
	if (IS_ERR(jz_pwm->pclk)) {
        pr_err("%s %d,get pclk error\n",
                __func__, __LINE__);
	    return -1;
	}
	printk("PCLK rate: %ld\n", clk_get_rate(jz_pwm->pclk));
#endif
