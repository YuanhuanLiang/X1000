...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	if(flag_coulomb){
		...
		if((charger->ocv >= 4100) && (charger->rest_cap < 100) && (charger->bat_current_direction == 0) && (charger->ext_valid) && charger->charge_on){
			if(axp_debug)
				DBG("%s->%d:(charger->ocv >= 4100) && (charger->rest_vol < 100) && (charger->bat_current_direction == 0) && (charger->ext_valid)\n",__FUNCTION__,__LINE__);
		...
		if((rt_rest_cap > (BATCAPCORRATE-1)) && (charger->rest_cap < BATCAPCORRATE) && charger->bat_current_direction == 0){
			if(axp_debug)
				DBG("%s->%d(rt_rest_cap > %d) && (charger->rest_cap < %d) && charger->bat_current_direction == 0\n",__FUNCTION__,__LINE__,BATCAPCORRATE-1,BATCAPCORRATE);
		...
		if(axp_debug)
			DBG(" first init charger->rest_cap = %d\n",charger->rest_cap);   
		/* full */
...
if(axp_debug){
	DBG("Cur_CoulombCounter = %d\n", Cur_CoulombCounter);
	DBG("charger->ic_temp = %d\n",charger->ic_temp);
	DBG("charger->vbat = %d\n",charger->vbat);
	DBG("charger->ibat = %d\n",charger->ibat);
	DBG("charger->disvbat = %d\n",charger->disvbat);
	DBG("charger->disibat = %d\n",charger->disibat);
	DBG("charger->vusb = %d\n",charger->vusb);
	DBG("charger->iusb = %d\n",charger->iusb);
	DBG("charger->vac = %d\n",charger->vac);
	DBG("charger->iac = %d\n",charger->iac);
	DBG("charger->ocv = %d\n",charger->ocv);
	DBG("charger->chgcur = %d\n",charger->chgcur);
	DBG("charger->chgearcur = %d\n",charger->chgearcur);
	DBG("charger->chgsuscur = %d\n",charger->chgsuscur);
	DBG("charger->chgclscur = %d\n",charger->chgclscur);
	DBG("rt_rest_cap = %d\n",rt_rest_cap);
	DBG("bat_cap = %d\n",bat_cap);
	DBG("base_cap = %d\n",base_cap);
	DBG("charger->ocv_rest_cap = %d\n",charger->ocv_rest_cap);
	DBG("charger->rest_cap = %d\n",charger->rest_cap);
...
#endif
	DBG("charger->is_on = %d\n",charger->is_on);
	DBG("charger->ext_valid = %d\n",charger->ext_valid);
	DBG("rdc_count = %d\n",rdc_count);
	DBG("state_count = %d\n",state_count);
	DBG("cap_count1 = %d,cap_count2 = %d,cap_count3 = %d,cap_count4 = %d\n",cap_count1,cap_count2,cap_count3,cap_count4);
#if defined (CONFIG_AXP_DEBUG)
	...
	if(charger->bat_det == 1){
		...
		if(flag_coulomb){
			...
			if(charger->bat_current_direction && (charger->rest_cap < pre_rest_cap))
				charger->rest_cap = pre_rest_cap;
			if(axp_debug)
				DBG("val = 0x%x,pre_rest_cap = %d,rest_cap = %d\n",val,pre_rest_cap,charger->rest_cap);
			/* full */
