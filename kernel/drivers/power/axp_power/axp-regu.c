...
static  int  vdd_core_vol =1200000;
...
static int axp_regulator_probe(struct platform_device *pdev)
{
	...
	if (ri == NULL) {
		dev_err(&pdev->dev, "invalid regulator ID specified,is correct with axp202\n");
		return -EINVAL;
	}
...
fs_initcall(axp_regulator_init);
