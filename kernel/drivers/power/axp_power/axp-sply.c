...
extern int g_charge_mode;
#endif
extern int dwc_vbus_status(void);

static int axp_battery_dbg_level = 0;
module_param_named(dbg_level, axp_battery_dbg_level, int, 0644);
...
#define DBG(format,args...)   \
	do { \
		if (axp_battery_dbg_level) { \
			printk(KERN_DEBUG "[AXP]"format,##args); \
		} \
	} while (0)
...
extern void usb_detect_interrupt_for_usb(int insert);
    usb_detect_interrupt_for_usb(charger->ext_valid);
    
	DBG("fun %s , line %d , charger->usb_valid  = %d , charger->ac_valid = %d,charger->charge_on=%d\n",
			__FUNCTION__, __LINE__, charger->usb_valid ,charger->ac_valid,charger->charge_on);
}
...
{
	DBG("%s  charger->bat_det=%d,charger->ext_valid=%d,charger->rest_cap=%d\n",__func__,charger->bat_det,charger->ext_valid,charger->rest_cap);
	if (charger->bat_det) {
		if (charger->ext_valid){
		...
		#else
			else if(charger->ac_det | charger->usb_det)
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
		#endif
			else
...
{
	DBG("battery change,charger->bat_det=%d\n",charger->bat_det);
	ssleep(2);
	axp_charger_update_state(charger);
	axp_charger_update(charger);
	if(charger->bat_det == 1){
		...
		DBG("%s....charger->rest_cap=%d,charger->ocv=%d\n",__func__,charger->rest_cap,charger->ocv);
        ...
        void *data)
{
	...
	DBG("pc_or_charger = %d,otg_phy_suspend = %d ,event=%x,irqs_enabled=%x\n",pc_or_charger,otg_phy_suspend,event,
	AXP_IRQ_ACIN|AXP_IRQ_USBIN|AXP_IRQ_ACOV|AXP_IRQ_USBOV|AXP_IRQ_CHAOV|AXP_IRQ_CHAST|AXP_IRQ_TEMOV|AXP_IRQ_TEMLO);
...
static int axp_battery_probe(struct platform_device *pdev)
{
	...
	/* \C9\E8\D6\C3pek\B0\B4\BC\FC\CF\FBϢ */
	...
	/* \C9\E8\D6\C3ȫ\BEֱ\E4\C1\BF\A3\AC\D3\C3\D3ڵ\F7\CA\D4 */
	axpcharger = charger;

	/* \B3\F5ʼ\BB\AF\C9\E8\D6\C3 */
	...
	/* \B3\F5ʼ\BB\AF\CF\D4ʾ\B5\E7\B3\D8\D0\C5Ϣ */
	axp_charger_update_state(charger);
	axp_charger_update(charger);
	DBG("%s......bat_det=%d,rest_cap=%d\n",__func__,charger->bat_det,charger->rest_cap);
	if(charger->bat_det == 1)
		axp_battery_infoinit(charger);

	/* \B4\B4\BD\A8\B8\FC\D0µ\E7\B3\D8\D0\C5Ϣ\B6\D3\C1\D0 */
	...
	/* δ\B4\F2\BF\AA\D6ж\CFʱ\A3\AC\C2\D6ѯ\BC\EC\B2\E2 */
...
#endif
	axp_set_rdcclose();
	/*\C9\E8\D6ùػ\FA\B3\E4\B5\E7\B5\E7\C1\F7*/
#if defined (CONFIG_AXP_CHGCURCHG) || defined(CONFIG_AXP_DEBUG)
