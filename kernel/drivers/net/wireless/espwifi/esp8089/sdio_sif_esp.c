...
#define WIFIWAKEUPGPIO      get_esp8089_gpio_wakeup()



static int wakeup_irq = 0;

static irqreturn_t irq_handler(int irq,void *dev)/* 中断处理函数,可以添加需要的动作 */

{

    esp_dbg(ESP_DBG_OP ,"xh %s,%d  irq = %d \n",__func__,__LINE__,irq);
...
static int wowlan_gpio_init(void)

{

    int ret;

    gpio_request( WIFIWAKEUPGPIO , "wifi_wakeup"); /*申请GPIO资源*/

    gpio_direction_input(WIFIWAKEUPGPIO); /*GPIO设置为输入模式*/

    wakeup_irq = gpio_to_irq(WIFIWAKEUPGPIO);

    if (wakeup_irq < 0){

        esp_dbg(ESP_DBG_OP, "xh Unable to get irq number for WIFIWAKEUPGPIO \n");
    ...
    ret = request_threaded_irq(wakeup_irq,irq_handler,NULL,IRQF_TRIGGER_RISING,"esp_wowlan_suppost",NULL);

    if (ret < 0){

        esp_dbg(ESP_DBG_OP, "xh failed to register irq %d\n",wakeup_irq);
    ...
    ret = enable_irq_wake(wakeup_irq);

    if (ret < 0 ){

        esp_dbg(ESP_DBG_OP, "xh failed to set gpio irq %d wake up!\n",wakeup_irq);
    ...
    esp_dbg(ESP_DBG_OP, "xh WIFIWAKEUPGPIO success wakeup_irq = %d\n",wakeup_irq);
...
static void wowlan_gpio_exit(void)
...
unsigned int esp_msg_level = ESP_DBG_ERROR | ESP_SHOW  | ESP_DBG_OP;
