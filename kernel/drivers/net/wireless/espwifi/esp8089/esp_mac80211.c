...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)) && defined(CONFIG_P2P_CONCURRENT)
...
static int esp_op_resume(struct ieee80211_hw *hw)
{
    ...
    sip_send_suspend_config(epub, 0);
		...
		struct ieee80211_if_conf *conf)
{
	...
	if(vif->type == NL80211_IFTYPE_AP){
		if((conf->changed & IEEE80211_IFCC_BEACON)){
			sip_send_bss_info_update(epub, evif, (u8*)conf->bssid, 2,0);
			//evif->beacon_interval = conf->beacon_int;
		}
		else{
	...
	if (vif->type == NL80211_IFTYPE_STATION) {
				...
				((changed & BSS_CHANGED_ASSOC) && (info->assoc)))
		{
			...
			if(sta == NULL) {
				...
				if(sta->ht_cap.ht_supported){
					highest_rate_idx = __builtin_clz(sta->ht_cap.mcs.rx_mask[0]) -24;
				} else {
					if(sta->supp_rates[IEEE80211_BAND_2GHZ] & (~(u32)CONF_HW_BIT_RATE_11B_MASK)){
						highest_rate_idx =  __builtin_clz(sta->supp_rates[IEEE80211_BAND_2GHZ]) - 20;
					}else {
						highest_rate_idx = __builtin_clz(sta->supp_rates[IEEE80211_BAND_2GHZ]) - 28;
					}
				}
			}
			...
			sip_send_bss_info_update(epub, evif, (u8*)info->bssid, info->assoc,highest_rate_idx);
		} else if ((changed & BSS_CHANGED_ASSOC) && (!info->assoc)) {
			ESP_IEEE80211_DBG(ESP_DBG_TRACE, " %s STA change disassoc\n", __func__);
			evif->beacon_interval = 0;
			memset(epub->wl.bssid, 0, ETH_ALEN);
			sip_send_bss_info_update(epub, evif, (u8*)info->bssid, info->assoc,0);
		} else {
		...
		if ((changed & BSS_CHANGED_BEACON_ENABLED) ||
			...
			if (info->enable_beacon && evif->ap_up != true) {
				...
				sip_send_bss_info_update(epub, evif, (u8*)info->bssid, 2,0);
				evif->ap_up = true;
...
#endif
{
	...
	static void
		esp_pub_init_mac80211(struct esp_pub *epub)
		{
			...
			/*add to support 11n*/
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29))
			epub->wl.sbands[IEEE80211_BAND_2GHZ].ht_cap.ht_supported = false;
...
#else
			epub->wl.sbands[IEEE80211_BAND_2GHZ].ht_info.ht_supported = false;
