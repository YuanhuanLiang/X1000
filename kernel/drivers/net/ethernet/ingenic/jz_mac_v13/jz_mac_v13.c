...
__attribute__((unused)) static void jzmac_set_multicast_list(struct net_device *dev)
{
...
static int jz_ethtool_get_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	struct jz_mac_local *priv = netdev_priv(dev);
	if (priv->phydev)
...
static int jz_ethtool_set_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	struct jz_mac_local *priv = netdev_priv(dev);
	if (priv->phydev)
...
static void jz_ethtool_get_drvinfo(struct net_device *dev,
			       struct ethtool_drvinfo *info)
{
	strlcpy(info->driver, JZMAC_DRV_NAME, sizeof(info->driver));
	strlcpy(info->version,JZMAC_DRV_VERSION, sizeof(info->version));
...
static const struct ethtool_ops jz_ethtool_ops = {
	.get_settings = jz_ethtool_get_settings,
	.set_settings = jz_ethtool_set_settings,
	.get_drvinfo = jz_ethtool_get_drvinfo,
	.get_link = ethtool_op_get_link,
	.get_ts_info = ethtool_op_get_ts_info,
};

static int bind_soc_ether_addr(unsigned char *dev_addr)
{
#ifdef CONFIG_JZ_EFUSE_V13
    char addr[16];
    printk("ethaddr set in soc chip id.\n");
    jz_efuse_id_read(1, (uint32_t *)addr);

    addr[0] &= 0xfe;    /* clear multicast bit */
    addr[0] |= 0x02;    /* set local assignment bit (IEEE802) */
    memcpy(dev_addr, &addr[16-ETH_ALEN], ETH_ALEN);
...
static int jz_mac_probe(struct platform_device *pdev)
{
	...
	if (bootargs_ethaddr) {
	    ...
	    if (bind_soc_ether_addr(ndev->dev_addr) < 0) {
	        /* MAC address:random */
	        printk("ethaddr set in Random.\n");
	...
	SET_ETHTOOL_OPS(ndev, &jz_ethtool_ops);
