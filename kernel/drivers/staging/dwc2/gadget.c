...
static void s3c_hsotg_init_fifo(struct dwc2_hsotg *hsotg)
{
	...
	val = addr << GDFIFOCFG_EPINFOBASE_SHIFT;
	val |= hsotg->fifo_mem;
	writel(val, hsotg->regs + GDFIFOCFG);
...
{
	struct device *dev = hsotg->dev;
	struct s3c_hsotg_plat *plat = NULL;
	int epnum;
	int ret;
	int i;
	...
	hsotg->g_rx_fifo_sz = (4 * 1 + 6) + 2 * (1024/4 + 1) + 2 * 8 + 1;	/*rx fifo size*/
	hsotg->g_np_g_tx_fifo_sz = 2 * (64 / 4);				/*ep0 tx fifo size*/
	memset(hsotg->g_tx_fifo_sz, 0, sizeof(hsotg->g_tx_fifo_sz));
	for (i = 1; i < hsotg->num_of_eps; i++) {				/*except ep0 tx fifo*/
		if (i == hsotg->num_of_eps - 1)
			hsotg->g_tx_fifo_sz[i] = 2 * (1024/4);			/*high-bandwidth endpoint*/
		else
			hsotg->g_tx_fifo_sz[i] = 2 * (512/4);
	}
	hsotg->g_using_dma = 1;
	/* Device tree specific probe */
	s3c_hsotg_of_probe(hsotg);
	/* Dump fifo information */
	dev_info(dev, "NonPeriodic TXFIFO size: %d\n",
						hsotg->g_np_g_tx_fifo_sz);
	dev_info(dev, "RXFIFO size: %d\n", hsotg->g_rx_fifo_sz);
	for (i = 0; i < MAX_EPS_CHANNELS; i++)
		dev_info(dev, "Periodic/Devicein TXFIFO%2d size: %d\n", i,
	...
	if (IS_ERR_OR_NULL(hsotg->uphy)) {
		...
		if (!plat) {
			dev_dbg(dev, "no platform data or transceiver defined\n");
			//return -EPROBE_DEFER;
		}
		hsotg->plat = plat;
	}
