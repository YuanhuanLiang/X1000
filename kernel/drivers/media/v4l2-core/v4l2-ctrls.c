...
const char *v4l2_ctrl_get_name(u32 id)
{
	...
	case V4L2_CID_CHD_GETREG:		return "get sensor register value";
	case V4L2_CID_CHD_SETREG:		return "set sensor register value";
			...
			struct v4l2_ext_control *c)
{
	...
	case V4L2_CTRL_TYPE_INTEGER:
		...
		return 0;
	case V4L2_CTRL_TYPE_BOOLEAN: // use this
		c->value = !!c->value;
		return 0;
...
printk("=====%s id: %x\n", __func__, qc->id);
	...
	qc->flags = ctrl->flags;
	qc->type = ctrl->type;
printk("name: %s min: %d max: %d  def: %d  step: %d\n", qc->name, qc->minimum, qc->maximum,qc->default_value,qc->step);
	return 0;
}
...
#if 0  // set every time  --add by liangyh 
	/* Don't set if there is no change */
	if (ret || !set || !cluster_changed(master))
		return ret;
#endif
	ret = call_op(master, s_ctrl); // 调用sensor驱动中的s_ctrl函数，如 sc031gs_handler_s_ctrl
	if (ret)
		return ret;
