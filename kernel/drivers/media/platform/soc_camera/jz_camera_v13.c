...
#ifdef CONFIG_SOC_CAMERA_DOUBLE_CHANNEL
#include "../../i2c/soc_camera/double_camera.h"
#endif
#define DRIVER_NAME "jz-cim"


#define DEBUG_FSC 0   // --liangyh
...
static int jz_init_dma(struct videobuf_queue *vq, struct videobuf_buffer *vbuf) {
	...
	if(icd->current_fmt->host_fmt->fourcc == V4L2_PIX_FMT_YUYV) {
		...
		dma_desc[vbuf->i].cmd = icd->sizeimage >> 2 |
			CIM_CMD_EOFINT | CIM_CMD_OFRCV;
	}else if(icd->current_fmt->host_fmt->fourcc == V4L2_PIX_FMT_GREY) {
		dma_desc[vbuf->i].cmd = icd->sizeimage >> 2 |
			CIM_CMD_EOFINT | CIM_CMD_OFRCV;
	} else {
...
static void jz_camera_wakeup(struct jz_camera_dev *pcdev,
	...
	dprintk(7, "after wake up cost %d ms count=%d\n", jiffies_to_msecs(jiffies - jiff_temp), vb->field_count);
}
...
static irqreturn_t jz_camera_irq_handler(int irq, void *data) {
	...
	struct jz_camera_dma_desc *dma_desc_paddr;
	struct videobuf_buffer *cur_vbuf = NULL;
...
#ifdef CONFIG_SOC_CAMERA_DOUBLE_CHANNEL
	sd = soc_camera_to_subdev(pcdev->icd[index]);
	db_camera_op = container_of(sd, struct double_camera_op, subdev);
#endif

	cur_vbuf = (struct videobuf_buffer *)(pcdev->vb_address[pcdev->dma_desc_head->id]);
...
#ifdef CONFIG_SOC_CAMERA_DOUBLE_CHANNEL
		cur_vbuf->field_count = db_camera_op->cur_channel<<1;
		db_camera_op->switch_work_channel(db_camera_op);
#endif
		spin_unlock_irqrestore(&pcdev->lock, flags);
...
static int camera_sensor_gpio_init(struct platform_device *pdev) {
	...
	for (i = 0; i < ARRAY_SIZE(pdata->cam_sensor_pdata); i++) {
		if (gpio_is_valid(pdata->cam_sensor_pdata[i].gpio_en)) {
			sprintf(gpio_name, "cim_enable%d", i);
			printk("cim_enable%d,%d\n", i,pdata->cam_sensor_pdata[i].gpio_en);
			ret = gpio_request(pdata->cam_sensor_pdata[i].gpio_en, gpio_name);
			if (ret){
				dev_err(&pdev->dev, "%s:request cim%d vdd en gpio fail\n",
						__func__, i);
				goto err_request_gpio_en;
			}
			gpio_direction_output(pdata->cam_sensor_pdata[i].gpio_en,
								  pdata->cam_sensor_pdata[i].gpio_en_level);
		}

		if (gpio_is_valid(pdata->cam_sensor_pdata[i].gpio_rst)) {
			...
			gpio_direction_output(pdata->cam_sensor_pdata[i].gpio_rst,
								  pdata->cam_sensor_pdata[i].gpio_rst_level);
		}

		if (gpio_is_valid(pdata->cam_sensor_pdata[i].gpio_power)) {
			...
			gpio_direction_output(pdata->cam_sensor_pdata[i].gpio_power,
								  pdata->cam_sensor_pdata[i].gpio_power_level);
		}

		if (gpio_is_valid(pdata->cam_sensor_pdata[i].ir_power_en)) {
			sprintf(gpio_name, "ir_power_en%d", i);
			printk("ir_power_en%d,%d\n", i,pdata->cam_sensor_pdata[i].ir_power_en);
			ret = gpio_request(pdata->cam_sensor_pdata[i].ir_power_en,
					gpio_name);
			if(ret) {
				dev_err(&pdev->dev, "%s:request cim%d gpio(ir power en) fail\n",
						__func__, i);
				goto err_request_ir_power_en;
			}
			gpio_direction_output(pdata->cam_sensor_pdata[i].ir_power_en,
								  pdata->cam_sensor_pdata[i].ir_power_en_level);
		}
		cnt++;
	}
...
err_request_ir_power_en:
	gpio_free(pdata->cam_sensor_pdata[cnt].gpio_power);
err_request_gpio_power:
	gpio_free(pdata->cam_sensor_pdata[cnt].gpio_rst);
err_request_gpio_rst:
	gpio_free(pdata->cam_sensor_pdata[cnt].gpio_en);
err_request_gpio_en:
	for (i = 0; i < cnt; ++i) {
		gpio_free(pdata->cam_sensor_pdata[i].gpio_en);
		gpio_free(pdata->cam_sensor_pdata[i].gpio_rst);
		gpio_free(pdata->cam_sensor_pdata[i].gpio_power);
		gpio_free(pdata->cam_sensor_pdata[i].ir_power_en);
	}
	return ret;
}
