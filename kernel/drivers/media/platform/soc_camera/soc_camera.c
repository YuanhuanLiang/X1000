...
const struct soc_camera_format_xlate *soc_camera_xlate_by_code(
    struct soc_camera_device *icd, unsigned int code)
{
    unsigned int i;
...
EXPORT_SYMBOL(soc_camera_xlate_by_code);
...
static int soc_camera_close(struct file *file)
{
	...
	if (!icd->use_count) {
		...
		pm_runtime_disable(&icd->vdev->dev);
		if (ici->ops->init_videobuf) {
			videobuf_mmap_free( (struct videobuf_queue *)(&icd->vb2_vidq) );
		}
...
static int soc_camera_probe(struct soc_camera_device *icd)
{
	...
	if (!v4l2_subdev_call(sd, video, g_mbus_fmt, &mf)) {
        ...
        icd->field       = mf.field;
        icd->current_fmt = soc_camera_xlate_by_code(icd, mf.code);
	}
