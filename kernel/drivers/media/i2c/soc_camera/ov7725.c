...
/* COM10 */
#define VSYNC_N         0x02   /* VSYNC negative */
#define HREF_R          0x08   /* HREF reverse */
#define PCLK_R          0x10   /* PCLK reverse */
#define HREF_2_HSYNC    0x40   /* HREF changes to HSYNC */
...
typedef struct
{
  u8 regAddr;     /*!< it use for what */
  u8 regVal;
}ov_camera;  /* end of struct define */
 ...
 */
static const ov_camera ov7725_15fps_pclk12Mhz[] = {
	{0x11, 0x03},
	{0x0d, 0x41},
	{0x2a, 0x00},
	{0x2b, 0x00},
...
static const ov_camera ov7725_25fps_pclk24Mhz[] = {
	{0x11, 0x01},
	{0x0d, 0x41},
	{0x2a, 0x00},
	{0x2b, 0x00},
...
static const ov_camera ov7725_30fps_pclk24Mhz[] = {
	{0x11, 0x01},
	{0x0d, 0x41},
	{0x2a, 0x00},
	{0x2b, 0x00},
...
static const struct ov772x_color_format ov772x_cfmts[] = {
	{
		.code		= V4L2_MBUS_FMT_YUYV8_2X8,		/* Y U Y V */
...
static int ov772x_write(struct i2c_client *client, u8 addr, u8 val)
{
	u8 buf[2] = { addr, val };
...
static int ov772x_write_array(struct i2c_client *client, const ov_camera *vals)
{
	int ret;

	while ((vals->regAddr != 0xff) || (vals->regVal != 0xff)) {
		ret = ov772x_write(client, vals->regAddr, vals->regVal);
		if (ret < 0) {
			dev_err(&client->dev, "array: 0x%02x, 0x%02x write failed",
			vals->regAddr, vals->regVal);
			return ret;
		}
		vals++;
	}
...
static int ov772x_s_ctrl(struct v4l2_ctrl *ctrl)		/* config reg here */
{
			     ...
			     const struct ov772x_win_size *win)
{
    ...
    /* set fps and pclk */
	ret = ov772x_write_array(client, ov7725_30fps_pclk24Mhz);
	if (ret < 0)
	...
	/* dummy size */
	reg_val = ov772x_read(client, EXHCH);
	reg_val |= ((win->rect.width & 3) << EXHCH_HSIZE_SHIFT) |
			   ((win->rect.height & 1) << EXHCH_VSIZE_SHIFT);
	ret = ov772x_write(client, EXHCH, reg_val);
	if (ret < 0)
		goto ov772x_set_fmt_error;

	/* COM3  CTRL3: YUYV */
	ret = ov772x_write(client, COM3, cfmt->com3);
	if (ret < 0)
		goto ov772x_set_fmt_error;
	ret = ov772x_write(client, DSP_CTRL3, cfmt->dsp3);
	if (ret < 0)
		goto ov772x_set_fmt_error;
	...
	/* polariry setting */
	ret = ov772x_write(client, COM10, VSYNC_N | PCLK_R);	/* plck reverse */
	if (ret < 0)
		goto ov772x_set_fmt_error;
...
static int ov772x_video_probe(struct ov772x_priv *priv)
{
	...
	if(ov772x_read(client, 0x29) != 0x50)
		printk("read != write, i2c check no pass \r\n");
	else
		printk("read == write, i2c check pass \r\n");
...
/* key function to set bus triggle type */
...
/* Jaly add */
static int ov772x_s_mbus_config(struct v4l2_subdev *sd,
...
static struct v4l2_subdev_video_ops ov772x_subdev_video_ops = {
	...
	.s_mbus_config	= ov772x_s_mbus_config,
			...
			const struct i2c_device_id *did)
{
	...
	{
		ov7725_info = (struct ov772x_camera_info *) kmalloc(sizeof(struct ov772x_camera_info), GFP_KERNEL);
		ov7725_info->flags = 0;
		ov7725_info->edgectrl.threshold = 0x08;	/* use default */
		ov7725_info->edgectrl.upper = 0x1F;		/* use default */
		ov7725_info->edgectrl.lower = 0x01;		/* use default */
		ov7725_info->edgectrl.strength = 0;		//OV772X_MANUAL_EDGE_CTRL, use auto;
	...
	if (ret < 0) {
		v4l2_ctrl_handler_free(&priv->hdl);	/* error */
	} else {
