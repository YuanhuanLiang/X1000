...
static struct device_id_struct device_id[XTX_DEVICES_NUM] = {
	DEVICE_ID_STRUCT(0xF2, "XT26G02B ", &xtx_param[0]),
};

static int32_t xtx_0b_get_read_feature(struct flash_operation_message *op_info) {
...
static int __init xtx_0b_nand_init(void) {
	...
	xtx_nand->ops.nand_read_ops.get_feature = xtx_0b_get_read_feature;
	return jz_sfcnand_register(xtx_nand);
}
module_init(xtx_0b_nand_init);
