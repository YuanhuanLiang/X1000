...
static int32_t is_split_page(const void * buf, uint32_t len) {
...
static int jz_sfcnand_write_oob(struct mtd_info *mtd, loff_t addr, struct mtd_oob_ops *ops)
{
	...
	if((ret = jz_sfc_nand_write(flash, pbuf, oob_addr / mtd->writesize, mtd->writesize, ops->ooblen))) {
...
static int jz_sfcnand_read(struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen, u_char *buf)
{
	...
	while(len) {
		...
		split = is_split_page(buf, rlen);
		if(!split) {
			pbuf = buf;
		} else {
			pbuf = nand_info->swapbuf;
		}
		ret = jz_sfc_nand_read(flash, pageaddr, columnaddr, pbuf, rlen);
		if(split)
...
static int jz_sfcnand_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf)
{
	...
	while(len) {
		...
		if((ret = jz_sfc_nand_write(flash, (u_char *)pbuf, pageaddr, columnaddr, wlen))) {
...
static int32_t jz_sfcnand_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	...
	if(ops->oobbuf) {
		...
		if(!split)
			pbuf = ops->oobbuf;
		ret = jz_sfc_nand_read(flash, pageaddr, mtd->writesize + ops->ooboffs, pbuf, ops->ooblen);
		if(split)
