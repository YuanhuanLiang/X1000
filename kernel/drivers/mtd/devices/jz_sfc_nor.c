...
#define UID_DIR 	"iduid"
#define UID_FILE 	"21"
unsigned char unid[8];
#endif
...
#if CONFIG_PROC_FS
int sfc_nor_read_unid(struct sfc_flash *flash, u8 command, unsigned int addr, int addr_len, size_t len, int dummy_byte)
{

	struct sfc_transfer transfer;
	int ret;
//	unsigned char unid[8] = {0};
...
#if 0	
	printk("sfc unid: %#x %#x %#x %#x %#x %#x %#x %#x \n", 
			unid[0], unid[1], unid[2], unid[3], unid[4], unid[5], unid[6], unid[7]);	
#endif
	return 0;
}

static struct proc_dir_entry *uid_dir, *proc_uid;

static int uid_proc_show(struct seq_file *file, void *v)
{
	seq_write(file, unid, sizeof(unid));
	return 0;
}

static int uid_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, uid_proc_show, NULL);
}

static struct file_operations uid_proc_ops = {
	.owner	 = THIS_MODULE,
	.open	 = uid_proc_open, 
	.read    = seq_read,     	/*linux/seq_file.c*/
	.release = single_release,  /*linux/seq_file.c*/
};

#if 0
static int chd_proc_create_entry(struct proc_dir_entry *parent)
{
	static struct proc_dir_entry *uid;
	static struct proc_dir_entry *aaa;
	int i = 0;
	char buf[8];
	
	// Create a interference entry 
	uid = create_proc_entry("uid", 0666, parent);  
	if (NULL == uid){
		return -1;
	}  
	...
	for(i = 0; i < 20; i ++){
		memset(buf, 0, 8);
		sprintf(buf, "%d", i);
		aaa = create_proc_entry(buf, 0666, parent);  
		if (NULL == aaa){
			return -1;
		}
...
static int chd_proc_write_uniqueID()
{	
	// Create user root dir under /proc 
	uid_dir = proc_mkdir(UID_DIR, NULL);
	if (NULL == uid_dir)  {  
		printk("m25p80: Create dir /proc/%s error!\n", UID_DIR);  
		return -1;  
	} 
	

	proc_uid = proc_create(UID_FILE, 0, uid_dir, &uid_proc_ops);
	if (NULL == proc_uid)  {  
		printk("m25p80: Create entry %s under /proc/%s error!\n",  UID_DIR, UID_FILE);  
		return -1;
	} 

//	chd_proc_create_entry(uid_dir);
	return 0;
}
#endif	
...
static int __init jz_sfc_probe(struct platform_device *pdev)
{
	...
	//#define OPCODE_RDUNID			0x4b
//	sfc_nor_read_id(flash, SPINOR_OP_RDID, 0, 0, 3, 0);

#if CONFIG_PROC_FS
	sfc_nor_read_unid(flash, 0x4b, 0, 4, 8, 0);
	chd_proc_write_uniqueID();
#endif
...
#if CONFIG_PROC_FS
	if(proc_uid){
		remove_proc_entry(UID_FILE, uid_dir);
	}
	if(uid_dir){
		remove_proc_entry(UID_DIR, NULL);
	}
#endif
