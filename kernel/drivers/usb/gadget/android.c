...
#define SERIAL_STR_VENDOR	"ingenic"
#ifdef CONFIG_BOARD_NAME
#define SERIAL_STR_DEVICE	CONFIG_BOARD_NAME
#else
#define SERIAL_STR_DEVICE	"board"
#endif
...
__attribute__((__unused__)) static int functionfs_ready_callback(struct ffs_data *ffs)
{
	struct android_dev *dev = _android_dev;
...
__attribute__((__unused__)) static void functionfs_closed_callback(struct ffs_data *ffs)
{
	struct android_dev *dev = _android_dev;
	struct functionfs_config *config = ffs_function.config;
...
__attribute__((__unused__)) static void *functionfs_acquire_dev_callback(const char *dev_name)
{
	return 0;
}

__attribute__((__unused__)) static void functionfs_release_dev_callback(struct ffs_data *ffs_data)
{
}
...
static struct fsg_module_parameters fsg_init_data = {
		.removable = {
			[0] = 1,
		},
		.luns = 1,
					...
					struct usb_composite_dev *cdev)
{
	...
	fsg_config_from_params(&config->fsg, &fsg_init_data);
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	sprintf(serial_str, "%s-%s-%d", SERIAL_STR_VENDOR, SERIAL_STR_DEVICE, get_random_int() % 100);
	/* Default strings - should be updated by userspace */
	strncpy(manufacturer_string, "Android", sizeof(manufacturer_string)-1);
	strncpy(product_string, "Android", sizeof(product_string) - 1);
	strncpy(serial_string, serial_str, sizeof(serial_string) - 1);
