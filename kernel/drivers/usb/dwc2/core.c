...
int dwc2_core_init(struct dwc2 *dwc)
{
	...
	if (!IS_ENABLED(CONFIG_DWC2_HOST_OVERCURRENT_DET)) {
	    /* host mode vbus-valid always valid
	     * This operation will cause increased sleep power consumption
	     */
...
static void dwc2_handle_common_interrupts(struct dwc2 *dwc, gintsts_data_t *gintr_status) {
	...
	if (gintr_status->b.usbsuspend) {
		...
		__pm_relax(dwc->jz_remote_wkup_pin->dwc2_ws);
    #ifdef CONFIG_JZ_USB_SUSPEND_REPORT_KEY
		/* suspend report key */
		jz_usb_remote_wkup_report_event();
    #endif
...
static irqreturn_t dwc2_interrupt(int irq, void *_dwc) {
	 ...
	 */
#ifdef CONFIG_JZ_USB_REMOTE_WKUP_PIN
	if(!dwc->jz_remote_wkup_pin->wkup_state) {
		__pm_stay_awake(dwc->jz_remote_wkup_pin->dwc2_ws);
	}
#endif
...
void jz_usb_remote_wkup_pin_interrupt(struct dwc2 *dwc)
{
	dwc2_interrupt(0,dwc);
}
...
#ifdef CONFIG_JZ_USB_REMOTE_WKUP_PIN
	ret = jz_usb_remote_wkup_pin_init(dwc);
	if (ret < 0) {
	    dev_err(dev, "failed to initialize remote wkup function\n");
	    goto fail_init_wkup;
	}
#endif
...
#ifndef CONFIG_JZ_USB_REMOTE_WKUP_PIN
	struct dwc2	*dwc = platform_get_drvdata(pdev);
	unsigned long	 flags;

	dwc2_spin_lock_irqsave(dwc, flags);
	dwc->suspended = 1;
#ifdef CONFIG_USB_DWC2_VERBOSE_VERBOSE // liangyh add just this line 2019-11-23
	dwc2_core_debug_en = 1;
#endif // liangyh add just this line 2019-11-23
