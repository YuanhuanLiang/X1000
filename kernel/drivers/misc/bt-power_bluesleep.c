...
#ifndef CONFIG_PRODUCT_X1000_IDRIVER
...
static int bt_rfkill_set_block(void *data, bool blocked)
{
    ...
    struct bt_rfkill_platform_data *pdata = data;

	if (pdata->power_state != blocked) {
        ret = pdata->power_switch(pdata, blocked);
	}
	return ret;
#else
	if (!first_called) {
...
static int __init_or_module bt_power_probe(struct platform_device *pdev)
{
	...
	if (bt_power_rfkill_probe(pdev)) {
		ret = -ENOMEM;
	}

	if(pdata && pdata->power_init){
        ret = pdata->power_init();
	}
...
static int bt_power_remove(struct platform_device *pdev)
{
    ...
    if(pdata->power_switch){
        ret = pdata->power_switch(pdata,0);
    }
	return ret;
#else
	bt_power_rfkill_remove(NULL);
	mutex_lock(&bt_power_lock);
	bt_power_state = 0;
