...
static int efuse_read_soc_id_proc(struct seq_file *m, void *v)
{
	int len = 0;
	unsigned int buf = 0;

	jz_efuse_read(TRIM3_ID, 2, &buf);
	len = seq_printf(m,"SOC ID: ");
	len += seq_printf(m,"%x",buf);
	len += seq_printf(m,"\n");
...
static int efuse_read_socID_proc_open(struct inode *inode,struct file *file)
{
	return single_open(file, efuse_read_soc_id_proc, PDE_DATA(inode));
}
...
static const struct file_operations efuse_proc_read_socID_fops ={
	.read = seq_read,
	.open = efuse_read_socID_proc_open,
	.write = NULL,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int jz_efuse_probe(struct platform_device *pdev)
{
	...
	res = proc_create("efuse_soc_id",0444,p,&efuse_proc_read_socID_fops);
	if(!res){
		pr_err("create proc of efuse_soc_id error!!!!\n");
	}
