...
{
    if (GPIO_WIFI_VDD != -1)
        gpio_direction_output(GPIO_WIFI_VDD, 0);
}

void wlan_pw_en_disable(void)
{
    if (GPIO_WIFI_VDD != -1)
        gpio_direction_output(GPIO_WIFI_VDD, 1);
}

int bcm_ap6212_wlan_init(void)
{
    if (GPIO_WIFI_VDD != -1) {
        if (gpio_request(GPIO_WIFI_VDD, "wifi_vddio")) {
            pr_err("ERROR: no wifi_vddio pin available !!\n");
            goto err_put_vddio;
        }
    }
    ...
    if (gpio_request(GPIO_WIFI_RST_N, "wifi_reset")) {
        ...
        gpio_direction_output(GPIO_WIFI_RST_N, 1);
    }
    bcm_ap6212_data.wifi_reset = GPIO_WIFI_RST_N;

    wake_lock_init(&bcm_ap6212_data.wifi_wake_lock, WAKE_LOCK_SUSPEND, "wifi_wake_lock");
