...
struct ili9342_power{
	struct regulator *vlcdio;
	struct regulator *vlcdvcc;
	int inited;
};

static struct ili9342_power lcd_power = {
	NULL,
	NULL,
	0
};

int ili9342_power_init(struct lcd_device *ld)
{
	int ret ;
	dprintf("======ili9342_power_init==============\n");
	if(GPIO_LCD_RST > 0){
		ret = gpio_request(GPIO_LCD_RST, "lcd rst");
		if (ret) {
			dprintf(KERN_ERR "can's request lcd rst\n");
			return ret;
		}
	}
	if(GPIO_LCD_CS > 0){
		...
		if (ret) {
			dprintf(KERN_ERR "can's request lcd cs\n");
			return ret;
		}
	}
	if(GPIO_LCD_RD > 0){
		...
		if (ret) {
			dprintf(KERN_ERR "can's request lcd rd\n");
			return ret;
		}
	}
	dprintf("set lcd_power.inited  =======1 \n");
	lcd_power.inited = 1;
	return 0;
}

int ili9342_power_reset(struct lcd_device *ld)
{
	if (!lcd_power.inited)
		return -EFAULT;
	dprintf("\t***RESET LCD\n");	
...
int ili9342_power_on(struct lcd_device *ld, int enable)
{
	if (!lcd_power.inited && ili9342_power_init(ld))
		return -EFAULT;
...
struct lcd_platform_data ili9342_pdata = {
	.reset    = ili9342_power_reset,
	.power_on = ili9342_power_on,
};

/* LCD Panel Device */
struct platform_device ili9342_device = {
	.name		= "ili9342_slcd",
	.dev		= {
		.platform_data	= &ili9342_pdata,
	},
};
...
static int backlight_init(struct device *dev)
{
	...
	if (ret) {
		dprintf(KERN_ERR "failed to request GPF for PWM-OUT1\n");
		return ret;
	}
#endif
	...
	if (ret) {
		dprintf(KERN_ERR "failed to reqeust BL PWR\n");
		return ret;
	}
