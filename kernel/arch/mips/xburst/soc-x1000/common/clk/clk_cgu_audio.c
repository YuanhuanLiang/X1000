...
#define     IIS_MCLK_64FS                0
#define     IIS_MCLK_128FS               1
#define     IIS_MCLK_256FS               3
#define     IIS_MCLK_384FS               5
#define     IIS_MCLK_512FS               7
#define     IIS_MCLK_768FS              11

typedef struct _i2s_sample_rate_config
{
    unsigned int parent_rate;
    unsigned int sample_rate;
    //unsigned m_n_od_p1sdiv;
    //unsigned cdr;
    unsigned int  M;//倍频
    unsigned int  N;//分频
    unsigned int DIV;//分频给BIT_CLK
    unsigned int sdiv_i2s;
} i2s_sample_rate_config;


static const i2s_sample_rate_config i2s_sample_rate_cfg[] =
{
    //parent_clock   fs   M      N  DIV
    {600000000,    8000,  32, 3125, 11, IIS_MCLK_768FS},
    {600000000,   16000,  64, 3125, 11, IIS_MCLK_512FS},
    {600000000,   24000,  64, 3125,  7, IIS_MCLK_512FS},
    {600000000,   32000, 128, 3125, 11, IIS_MCLK_768FS},
    {600000000,   48000, 128, 3125,  7, IIS_MCLK_512FS},
    {600000000,   96000, 128, 3125,  3, IIS_MCLK_256FS},
    {600000000,  192000, 128, 3125,  1, IIS_MCLK_128FS},

    {600000000,   11025,  30, 3181,  7, IIS_MCLK_512FS},
    {600000000,   22050,  41, 2179,  7, IIS_MCLK_512FS},
    {600000000,   44100,  75, 1993,  7, IIS_MCLK_512FS},
    {600000000,   88200,  75, 1993,  3, IIS_MCLK_256FS},
    {600000000,  176400,  75, 1993,  1, IIS_MCLK_128FS},
};
#endif
...
static int cgu_audio_calculate_set_rate(struct clk* clk, unsigned long rate, unsigned int pid){
    ...
    if(pid == CLK_ID_MPLL){
        ...
        parent_clk = get_clk_from_id(CLK_ID_MPLL);
    }
    else if(pid == CLK_ID_SCLKA){
        audio_div = (int*)audio_div_apll;
        parent_clk = get_clk_from_id(CLK_ID_SCLKA);
    }
    else
        return 0;
    ...
    if(no == CGU_AUDIO_I2S){
        ...
        for(i=0; i<ARRAY_SIZE(i2s_sample_rate_cfg); i++){
            if(i2s_sample_rate_cfg[i].parent_rate == parent_rate &&
               i2s_sample_rate_cfg[i].sample_rate == rate)
                break;
        }

        if( i == ARRAY_SIZE(i2s_sample_rate_cfg) )
        {
            printk("cgu aduio set rate err parent rate:%lu, sample_rate:%lu!\n", parent_rate, rate);
            return -EINVAL;
        }

        m = i2s_sample_rate_cfg[i].M;
        n = i2s_sample_rate_cfg[i].N;
        d = i2s_sample_rate_cfg[i].DIV;
        ...
        if(tmp_val&cgu_clks[no].en){
            ...
            cpm_outl((1 << 30) | (n / 2),cgu_clks[CGU_AUDIO_I2S1].off);
        }else{
            cgu_clks[no].cache = tmp_val;
            cpm_outl((1 << 30) | (n / 2),cgu_clks[CGU_AUDIO_I2S1].off);
        }
        writel(d,(volatile unsigned int*)I2S_PRI_DIV);
        spin_unlock_irqrestore(&cpm_cgu_lock,flags);
    }else if (no == CGU_AUDIO_PCM){
        for(i=0;i<50;i+=3){
            if(audio_div[i] == rate)
