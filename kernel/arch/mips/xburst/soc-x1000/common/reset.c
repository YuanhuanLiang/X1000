...
#ifdef CONFIG_WKUP_PIN_RST
static void wkup_pin_long_press_reset(void)
{
    unsigned int rtc_wkup = inl(RTC_IOBASE + RTC_PWRONCR);

    rtc_wkup &= ~(0xff << 0);
    rtc_wkup |= (CONFIG_JUDGE_TIME << 4 | 0x0b);

    rtc_write_reg(RTC_PWRONCR, rtc_wkup);
}
#endif
