...
#ifdef GPIO_PM_DEBUG
void dump_port_status (unsigned int port) {
    unsigned pxpin =  *(volatile unsigned int *) (0xb0010000 + port * 0x100 );
    unsigned pxint =  *(volatile unsigned int *) (0xb0010000 + port * 0x100  + 0x10);
    unsigned pxmsk  =  *(volatile unsigned int *) (0xb0010000 + port * 0x100  + 0x20);
    unsigned pxpat1 =  *(volatile unsigned int *) (0xb0010000 + port * 0x100  + 0x30);
    unsigned pxpat0 =  *(volatile unsigned int *) (0xb0010000 + port * 0x100  + 0x40);
    unsigned pxpen =  *(volatile unsigned int *) (0xb0010000 + port * 0x100  + 0x70);
...
int __init gpio_nc_init(void)
{
    unsigned int i, state, port, ret, gpio;
    struct jzgpio_chip *jz;

    for(i = 0; gpio_nc_table[i][1] != GSS_TABLET_END;i++) {
        gpio = gpio_nc_table[i][0];
        port = gpio_nc_table[i][0] / 32;
        state = gpio_nc_table[i][1];

        if(port >= GPIO_NR_PORTS)
        {
            printk("ERROR: gpio_nc_table[%d] is invalid!\n",i);
            continue;
        }

        jz = &jz_gpio_chips[port];

        ret = gpio_request(gpio_nc_table[i][0], "NC_PIN");
        if (ret)
            printk("%s: error to request gpio: %d\n", __func__, gpio_nc_table[i][0]);
        ...
        case GSS_INPUT_PULL:
            ...
            writel(BIT(gpio % 32), jz->reg + PXPENC);
            break;

        case GSS_INPUT_NOPULL:
            gpio_direction_input(gpio);
            writel(BIT(gpio % 32), jz->reg + PXPENS);
            break;
...
int __init gpio_ss_check(void)
{
	...
	for(i = 0; gpio_ss_table[i][1] != GSS_TABLET_END;i++) {
		...
		case GSS_IGNORE:
			...
			break;
		case GSS_INT_LO:
			gpio_type = GPIO_INT_LO;
			break;
		case GSS_INT_HI:
			gpio_type = GPIO_INT_HI;
			break;
		case GSS_INT_FE:
			gpio_type = GPIO_INT_FE;
			break;
		case GSS_INT_RE:
			gpio_type = GPIO_INT_RE;
			break;
		}
