/* *  jz_tx_codec.c - Linux kernel modules for audio codec
 *
...
static int codec_set_buildin_mic(void)
{
	...
	if(mic_gain){
		...
		codec_reg_set(TS_CODEC_CMICCR_22, 1, 2, 1);
		msleep(10);
	}
...
int dump_codec_regs(void)
{
	...
	printk("Dump TX Codec register End.\n");
...
#define INTERNAL_CODEC_CLOCK 12000000
int jz_codec_init(void)
{
	...
	codec_dev->codec_clk = INTERNAL_CODEC_CLOCK;
