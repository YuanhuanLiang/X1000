...
static int soc_bind_dai_link(struct snd_soc_card *card, int num)
{
    ...
    char tmp_codec_name[NAME_SIZE];
    char tmp_dai_codec_name[NAME_SIZE];
#endif
#endif
	...
	list_for_each_entry(codec, &codec_list, list) {
		if (dai_link->codec_of_node) {
            ...
            strlcpy(p_codec_name, codec->name, NAME_SIZE);
            strlcpy(p_dai_codec_name, dai_link->codec_name, NAME_SIZE);

            p_codec_name = strsep(&p_codec_name,".");
            p_dai_codec_name = strsep(&p_dai_codec_name,".");

            if( p_codec_name && p_dai_codec_name )
            {
                if( strcmp(p_codec_name, p_dai_codec_name) )
                {
                    continue;
                }
            }
