...
struct jz_dma_pcm {
	...
	dma_addr_t tmp_dma_buf_phys; /* for aic record mono mode.*/
	void *     tmp_dma_buf_virt;
	dma_addr_t tmp_dma_buf_last_pos;
};
...
static size_t
snd_pcm_get_tmp_dma_buf_pos_algin_period(struct snd_pcm_substream *substream, dma_addr_t addr)
{
	struct snd_soc_pcm_runtime *rtd = snd_pcm_substream_chip(substream);
	struct jz_dma_pcm *jz_pcm = snd_soc_platform_get_drvdata(rtd->platform);
	return (addr - jz_pcm->tmp_dma_buf_phys -
			(addr - jz_pcm->tmp_dma_buf_phys)%
...
static int is_record_mono(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = snd_pcm_substream_chip(substream);
	/* FIXME: only support S16_LE now. */
	if ((!strcmp(rtd->cpu_dai->name, "jz-asoc-aic-i2s"))
...
static void jz_asoc_dma_callback(void *data)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (is_record_mono(substream)) {
			struct snd_soc_pcm_runtime *rtd = snd_pcm_substream_chip(substream);
			struct jz_dma_pcm *jz_pcm = snd_soc_platform_get_drvdata(rtd->platform);
			dma_addr_t tmp_dma_buf_size = snd_pcm_lib_buffer_bytes(substream) * 2;
			dma_addr_t tmp_dma_buf_curr_pos =
				snd_pcm_get_tmp_dma_buf_pos_algin_period(substream, pdma_addr);

#define BUF_COPY(TYPE, POS, COUNT)							\
do { 											\
	TYPE *src = (TYPE*)((jz_pcm->tmp_dma_buf_phys | 0xa0000000) + (POS));		\
	TYPE *dst = (TYPE*)((substream->runtime->dma_addr | 0xa0000000) + (POS)/2);	\
	sample_count = (COUNT)/2/sizeof(TYPE);						\
...
#define DO_TMP_BUF_COPY(TYPE)							\
do { 										\
	unsigned int i, sample_count; 						\
	if ( tmp_dma_buf_curr_pos > jz_pcm->tmp_dma_buf_last_pos ) {			\
		BUF_COPY(TYPE, jz_pcm->tmp_dma_buf_last_pos,			\
				tmp_dma_buf_curr_pos - jz_pcm->tmp_dma_buf_last_pos);	\
	} else {								\
		BUF_COPY(TYPE, jz_pcm->tmp_dma_buf_last_pos,			\
				tmp_dma_buf_size - jz_pcm->tmp_dma_buf_last_pos);	\
		BUF_COPY(TYPE, 0, tmp_dma_buf_curr_pos);				\
	}									\
} while(0);
			switch (substream->runtime->format) {
			/* FIXME: stereo convert to mono mode, only support S16_LE now. */
			case SNDRV_PCM_FORMAT_S16_LE:
				DO_TMP_BUF_COPY(short);
				break;
			default:
				DO_TMP_BUF_COPY(int);
				break;
...
#undef BUF_COPY
#undef DO_TMP_BUF_COPY
			prtd->pos = tmp_dma_buf_curr_pos / 2;
			jz_pcm->tmp_dma_buf_last_pos = tmp_dma_buf_curr_pos;
		} else {
...
static int jz_asoc_dma_prepare_and_submit(struct snd_pcm_substream *substream)
{
	...
	if (is_record_mono(substream)) {
		desc = prtd->dma_chan->device->device_prep_dma_cyclic(prtd->dma_chan,
				jz_pcm->tmp_dma_buf_phys,
				snd_pcm_lib_buffer_bytes(substream) * 2,
				snd_pcm_lib_period_bytes(substream) * 2,
				direction,
				flags,
				NULL);
		jz_pcm->tmp_dma_buf_last_pos = 0;
	} else {
		desc = prtd->dma_chan->device->device_prep_dma_cyclic(prtd->dma_chan,
...
static void jz_pcm_free(struct snd_pcm *pcm)
{
	...
	if (jz_pcm && jz_pcm->tmp_dma_buf_virt)
		dma_free_coherent(
				jz_pcm->chan[SNDRV_PCM_STREAM_CAPTURE]->device->dev,
				JZ_DMA_BUFFERSIZE * 2,
				jz_pcm->tmp_dma_buf_virt, jz_pcm->tmp_dma_buf_phys);
	return;
}
...
static int jz_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (!strcmp(rtd->cpu_dai->name, "jz-asoc-aic-i2s")) {
		jz_pcm->tmp_dma_buf_virt = dma_alloc_coherent(
				jz_pcm->chan[SNDRV_PCM_STREAM_CAPTURE]->device->dev,
				JZ_DMA_BUFFERSIZE * 2,
				&jz_pcm->tmp_dma_buf_phys, GFP_KERNEL);
		if (!jz_pcm->tmp_dma_buf_virt) {
			dev_err(rtd->dev, "alloc dma area for icdc-d3 failed!\n");
			goto out;
		}
	}
	return 0;
