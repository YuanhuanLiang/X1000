		    ...
		    size_t len, u8 mac[16])
{
	...
	char __desc_desc[sizeof(struct shash_desc) + crypto_shash_descsize(tfm)] CRYPTO_MINALIGN_ATTR;
	struct shash_desc *desc = (struct shash_desc *)__desc_desc;
	int err;
...
static u8 smp_random(struct smp_chan *smp)
{
	...
	if (memcmp(smp->pcnf, confirm, sizeof(smp->pcnf))) {
