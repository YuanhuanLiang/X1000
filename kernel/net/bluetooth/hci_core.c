...
struct adv_info *hci_get_next_instance(struct hci_dev *hdev, u8 instance)
{
	...
	if (cur_instance == list_entry(hdev->adv_instances.prev,
					    struct adv_info, list))
		return list_entry(hdev->adv_instances.next,
						 struct adv_info, list);
	else
		return list_entry(cur_instance->list.next, typeof(*(cur_instance)), list);
}
 ...
 */
static void kfree_const(const void *x)
{
 ...
 */
static const char *kstrdup_const(const char *s, gfp_t gfp)
{
 ...
 */
static const char *kvasprintf_const(gfp_t gfp, const char *fmt, va_list ap)
{
	if (!strchr(fmt, '%'))
