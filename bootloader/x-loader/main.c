...
void x_loader_main(void) {
    ...
    /*
     *  enter hibernate to power down,
     *  depend on CONFIG_HIBERNATE_WKUP_KEY status
     */
    unsigned int flag = 0;
    flag = cpm_get_scrpad();

    gpio_direction_input(CONFIG_HIBERNATE_WKUP_KEY);
    if (gpio_get_value(CONFIG_HIBERNATE_WKUP_KEY)) {
        /*
         * press down key hold time to reset cpu when it power down(ms)
         */
        if ((flag & 0xffff) != RECOVERY_SIGNATURE) {
            printf("PowerOFF,Press WKUP key to run.\n");
            enter_hibernate(2000);
        }
    }
