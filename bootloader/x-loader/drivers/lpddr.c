...
static void sdram_resize_to_x1500la(void) {
    uint32_t ddrc_cfg = 0;

    ddrc_cfg = ddr_readl(DDRC_CFG);

    ddrc_cfg &= ~((0x3f << 24) | (0x3f << 8));
    ddrc_cfg |= ((0x01 << 24) | (0x01 << 8));

    ddr_writel(ddrc_cfg, DDRC_CFG);
    ddr_writel(0x000020ff, DDRC_MMAP0);
    ddr_writel(0x00002100, DDRC_MMAP1);
...
static void sdram_resize_to_x1500l(void) {
    uint32_t ddrc_cfg = 0;
...
static void sdram_resize_to_x1501(void) {
    uint32_t ddrc_cfg = 0;
...
static void probe_sdram_size(void) {
    ...
    switch(cpu_socid) {
        case X1501:
            sdram_resize_to_x1501();
        case X1500L:
            sdram_resize_to_x1500l();
            break;
        case X1500LA:
            sdram_resize_to_x1500la();
            break;
        case X1500:
        case X1500_NEW:
        case X1000:
