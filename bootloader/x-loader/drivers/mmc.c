...
static int mmc_polling_status(uint32_t status) {
    ...
    if (!timeout) {
        msc_debug("polling status(0x%x) timeout, MSC_STAT=0x%x\n", status,
                msc_readl(MSC_STAT));
       return -1;
    }

    if (msc_readl(MSC_STAT) & ERROR_STAT) {
        msc_debug("polling status(0x%x) error, status=0x%x\n", status,
                msc_readl(MSC_STAT));
        return -1;
    }
...
static int mmc_switch(unsigned char index, unsigned char value)
{
    struct mmc_cmd cmd;
    int error = 0;
...
static int mmc_card_starup(void) {
    ...
    if (IS_SD(mmc)) {
        ...
        /*
         * =0: backwards compatibility iterface timing ( clock < 26MHz)
         * =1: High Speed ( clock 52MHz)
         * =2: HS200 (clock 200MHz)
         * =3: HS400 (clock 200MHz Double-Data-Rate 400MHz)
         */
        error = mmc_switch(EXT_CSD_HS_TIMING, 1);
        if (error < 0)
            return error;

        error = mmc_switch(EXT_CSD_BUS_WIDTH, EXT_CSD_BUS_WIDTH_4);
        if (error < 0)
            return error;
